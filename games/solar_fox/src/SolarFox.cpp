...
namespace games {
    ...
    void SolarFox::loadClassFromChar(char c, long int x, long int y)
    {
        ...
        switch (c) {
            case '1': m_objectToDraw.push_back(new Monster(coord, std::make_pair(0, 1))); static_cast <Monster *> (m_objectToDraw[m_objectToDraw.size()-1])->setDirectionLazer(RIGHT); break;
            case '2': m_objectToDraw.push_back(new Monster(coord, std::make_pair(-1, 0))); static_cast <Monster *> (m_objectToDraw[m_objectToDraw.size()-1])->setDirectionLazer(DOWN); break;
            case '3': m_objectToDraw.push_back(new Monster(coord, std::make_pair(0, -1))); static_cast <Monster *> (m_objectToDraw[m_objectToDraw.size()-1])->setDirectionLazer(LEFT); break;
            case '4': m_objectToDraw.push_back(new Monster(coord, std::make_pair(1, 0))); static_cast <Monster *> (m_objectToDraw[m_objectToDraw.size()-1])->setDirectionLazer(UP); break;
    ...
    std::vector <IObjectToDraw *> SolarFox::getAssets() const
    {
        ...
        assets.push_back(new Monster(default_coord, default_coord, "monster", "./games/solar_fox/assets/monster/"));
